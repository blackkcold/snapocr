...
public struct TextTool: Sendable {
  ...
  /// Creates a text annotation tool.
  public init() {}

  /// Returns the tight rendered size, including the tool's visual padding.
  ///
  /// - Parameters:
  ///   - node: The text annotation to measure.
  ///   - maximumWidth: An optional maximum width in image pixels. Text wraps when its
  ///     natural width exceeds this value.
  /// - Returns: The measured text size in image pixels, including visual padding.
  public func suggestedSize(for node: AnnotationNode, maximumWidth: CGFloat? = nil) -> CGSize {
    ...
    let contentWidth =
      maximumWidth.map {
        max(($0 - horizontalPadding) / horizontalScale, 1)
      } ?? 100_000
    ...
    let measured = CTFramesetterSuggestFrameSizeWithConstraints(
      ...
      nil,
      CGSize(width: contentWidth, height: 100_000),
      &fittedRange
    )

    let measuredSize = CGSize(
      width: ceil(measured.width * horizontalScale) + horizontalPadding,
      height: ceil(measured.height) + 8
    )
    guard let maximumWidth else { return measuredSize }
    return CGSize(width: min(measuredSize.width, maximumWidth), height: measuredSize.height)
  }
  ...
  public func render(node: AnnotationNode, in context: CGContext, imageSize: CGSize) {
    ...
    let attributedString = NSAttributedString(string: text, attributes: attributes)
    let framesetter = CTFramesetterCreateWithAttributedString(attributedString)
    let suggestedSize = suggestedSize(for: node, maximumWidth: max(imageSize.width - position.x, 1))
    let textRect =
      node.normalizedRect == .zero
      ? CGRect(
        x: position.x,
        y: position.y,
