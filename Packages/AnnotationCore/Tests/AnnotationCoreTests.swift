...
struct AnnotationCoreTests {
  ...
  @Test func longImagePolicyAlwaysEnablesScrollingCaptures() {
    #expect(
      LongImageEditingPolicy.supportsVerticalTrim(
        imageWidth: 1_920,
        imageHeight: 1_080,
  ...
  @Test func textSuggestedSizeWrapsWithinMaximumWidth() {
    let tool = TextTool()
    let node = AnnotationNode(
      tool: .text,
      text: Array(repeating: "wrapping", count: 12).joined(separator: " "),
      fontSize: 24
    )

    let naturalSize = tool.suggestedSize(for: node)
    let constrainedSize = tool.suggestedSize(for: node, maximumWidth: 160)

    #expect(constrainedSize.width <= 160)
    #expect(constrainedSize.height > naturalSize.height * 2)
  }
